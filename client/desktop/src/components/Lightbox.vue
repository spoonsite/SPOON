<template>
  <div>
    <!-- MEDIA carousel -->
    <button type="button" v-for="(item, index) in list" :key="item.link" class="imageSizing" @click="lightboxOn(index)">
      <img
      ...
      ></video>
      <p v-if="index === 0" class="image-number">Image 1 of {{ list.length }}</p>
    </button>
        ...
        <div class="lightboxImageWrapper">
          <transition name="swipe" keep-alive mode="out-in">
            <div :key="currentItem.link" style="margin-bottom: 3em;">
              ...
              <img
                v-if="currentItem.mediaTypeCode === 'IMG'"
                :src="baseURL + currentItem.link"
                class="lightboxImage elevation-6"
              />
              <video
                controls
                v-else-if="currentItem.mediaTypeCode === 'VID'"
                :src="currentItem.link"
                class="lightboxImage elevation-6"
              ></video>
              <p style="color: white;">
                Image {{ currentIndex + 1 }} of {{ list.length }}
                <v-btn dark small flat icon :href="baseURL + currentItem.link"
                  ><v-icon class="download-icon">fas fa-download</v-icon></v-btn
                >
              </p>
            </div>
          </transition>
