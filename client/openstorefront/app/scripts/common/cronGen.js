...
(function ($) {
  ...
  $.fn.extend({
    cronGen: function () {
      //create top menu
      var cronContainer = $("<div/>", { id: "CronContainer", style: "width:480px;height:300px; background: white;" });
      var mainDiv = $("<div/>", { id: "CronGenMainDiv", style: "width:480px;height:270px;" });
      var topMenu = $("<ul/>", { "class": "nav nav-tabs", id: "CronGenTabs" });
      var generateButton = $('<button type="button" class="btn btn-default" id="genButton" style="position:absolute; bottom:10px; left: 10px;">Save</button>');
      ...
      var container = $("<div/>", { "class": "container-fluid", "style": "margin-top: 10px; padding-left: 10px; padding-right: 10px;" });
      ...
      $(cronContainer).append(generateButton);

      $(this).append(cronContainer);
      var that = $(this);
      ...
      that.fillInMonths = function () { //
        ...
        $(".months").each(function () {
          that.fillOptions(this, days);
        });
      };

      that.fillOptions = function (elements, options) {
        for (var i = 0; i < options.length; i++)
          $(elements).append("<option value='" + options[i].val + "'>" + options[i].text + "</option>");
      };
      that.fillDataOfMinutesAndHoursSelectOptions = function () {
        for (var i = 0; i < 60; i++) {
          if (i < 24) {
            $(".hours").each(function () { $(this).append(that.timeSelectOption(i)); });
          }
          $(".minutes").each(function () { $(this).append(that.timeSelectOption(i)); });
        }
      };
      that.fillInWeekDays = function () {
        var days = [
        { text: "Monday", val: "MON" },
      ...
      that.fillDayWeekInMonth = function () {
        var days = [
        { text: "First", val: "1" },
        { text: "Second", val: "2" },
        { text: "Third", val: "3" },
      ...
      that.displayTimeUnit = function (unit) {
        if (unit.toString().length == 1)
          return "0" + unit;
        return unit;
      };
      that.timeSelectOption = function (i) {
        return "<option id='" + i + "'>" + that.displayTimeUnit(i) + "</option>";
      };

      that.populate = function(cron){
        return;
      }
      ...
      that.fillDataOfMinutesAndHoursSelectOptions();
      that.fillDayWeekInMonth();
      that.fillInWeekDays();
      that.fillInMonths();
      ...
      $('#genButton').on('click', function (e) {
        that.generate();
        var triggerChange = new CustomEvent(
          "cronChange", 
          {
            'detail':{
              'cron': that.generate(),
        ...
        that[0].dispatchEvent(triggerChange);
      })
      return that;
    }
  });
