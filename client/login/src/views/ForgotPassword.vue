  ...
  <v-layout ma-3>
    <v-flex xs12 md4 offset-md4 sm6 offset-sm3>
      <v-card class="elevation-5 mt-3 mx-3">
        ...
        <v-card-actions>
          <v-btn :loading="loading" :disabled="!valid" block color="secondary" @click="submitReset()">Send Email Approval</v-btn>
        </v-card-actions>
      </v-card>
    </v-flex>
  </v-layout>
  ...
  data: function () {
    return {
      ...
      password1Rules: [
        ...
        v => {
          const regex = new RegExp('^(?=.*[A-Z])(?=.*\\d)(?=.*[~`!@#$%^&*()-+=<>:;"\',.?])[A-Za-z\\d~`!@#$%^&*()-+=<>:;"\',.?]{' + String(this.$store.state.securitypolicy.minPasswordLength) + ',}$')
          return regex.test(v) ||
