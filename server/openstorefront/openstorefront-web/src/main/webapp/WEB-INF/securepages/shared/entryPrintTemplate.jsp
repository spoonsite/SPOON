...
<tpl
	if="!show.evaluation || !evaluation || !evaluation.evaluationSections || !evaluation.evaluationSections.length &gt; 0">
...
<tpl if="show.description">
	<div class="print-left-block print-section">
		<h3>
			<tpl if="componentSecurityMarkingType">({componentSecurityMarkingType}) </tpl>Description:
		</h3>
		{description}
	</div>
</tpl>
		...
		<div class="print-left-block print-section">
			...
			<table class="print-table" style="width: 100%">
				<tpl for="dependencies">
					<tr class="print-table">
						<td class="print-table">
							<tpl if="securityMarkingType">({securityMarkingType}) </tpl><b>{dependencyName}
								{version}</b><br>
							{comment}
						</td>
					</tr>
			...
			<tpl for="reviews">
				<table style="width:100%">
					<tpl if="activeStatus == 'P'">
						<tr>
							<td colspan="3" class="alert-warning"
								style="text-align: center; font-size: 1.5em; font-weight: bold;"><i
									class="fa fa-warning"></i> Review pending moderator approval before being made
								public.</td>
						</tr>
					</tpl>
					<tr>
						<td valign="top">
							<h1>
								<tpl if="securityMarkingType">({securityMarkingType}) </tpl>{title} <br>
								<tpl for="ratingStars"><i class="fa fa-{star} rating-star-color"></i></tpl>
							</h1>
							<div class="review-who-section">{username} ({userTypeCode}) -
								{[Ext.util.Format.date(values.updateDate, "m/d/y")]}<tpl if="recommend"> -
									<b>Recommend</b></tpl>
							</div><br>
							<b>Organization:</b> {organization}<br>
...
<tpl if="show.questions">
	<tpl if="questions && questions.length &gt; 0">
		<div class="pageBreak">
			...
			<tpl for="questions">
				<tpl if="activeStatus == 'P'">
					<div class="alert-warning" style="text-align: center; font-size: 1.5em; font-weight: bold;"><i
							class="fa fa-warning"></i> Question pending moderator approval before being made public.
					</div>
				</tpl>
				<div class="question-question"><span class="question-response-letter-q">Q.</span>
					<tpl if="securityMarkingType">({securityMarkingType}) </tpl><b>{question}</b>
				</div>
				...
				<div style="padding-left: 10px; padding-right: 10px;">
					<tpl for="responses">
						<tpl if="activeStatus == 'P'">
							<div class="alert-warning" style="text-align: center; font-size: 1.5em; font-weight: bold;">
								<i class="fa fa-warning"></i> Answer pending admin approval before being made public.
							</div>
						</tpl>
						<div class="question-response"><span class="question-response-letter">A.</span>
							<tpl if="securityMarkingType">({securityMarkingType}) </tpl>{response}
						</div>
						<div class="question-info">{username} ({userType}) - {[Ext.util.Format.date(values.answeredDate,
							"m/d/Y")]}</div><br>
						<hr>
					</tpl>
				</div>
	...
	<tpl if="componentMedia && componentMedia.length &gt; 0">
		<div class="pageBreak">
			...
			<tpl for="componentMedia">
				...
				<tpl if="mediaTypeCode != 'IMG'">
					<b>Non-Printable:</b> {contentType}<tpl if="securityMarkingType">({securityMarkingType}) </tpl>
					<tpl if="caption"> - {caption}</tpl>
					<tpl if="originalFileName"> - {originalFileName}</tpl><br>
				</tpl>
				<br>
			</tpl>
		</div>
...
<tpl for="fullEvaluations">
	<div class="pageBreak" style="clear: both;">
		<tpl if="parent.show['evaluationDetails'+evaluation.version]">
			...
			<!-- Reusability Factors -->
			<tpl
				if="parent.show['evalReusability'+evaluation.version] && evaluationScores && evaluationScores.length &gt; 0">
				<h3 class="quickView toggle-collapse">Reusability Factors (5=best) <div data-qtip="Expand panel"
						style="float: right;" data-ref="toolEl"
						class=" x-tool-tool-el x-tool-img x-tool-expand-bottom eval-toggle-caret" role="presentation">
					</div>
				</h3>
		...
		<tpl
			if="parent.show['evalRecommendations'+evaluation.version] && checkListAll.recommendations && checkListAll.recommendations.length &gt; 0">
			<h3 class="quickView toggle-collapse">Evaluation Recommendations <div data-qtip="Expand panel"
					style="float: right;" data-ref="toolEl"
					class=" x-tool-tool-el x-tool-img x-tool-expand-bottom eval-toggle-caret" role="presentation"></div>
			</h3>
			<table class="quickView-table" width="100%">
				<tr>
					<th class="details-table">Type</th>
					<th class="details-table">Recommendation</th>
					<th class="details-table">Reason</th>
				</tr>
		...
		<!-- Content Sections -->
		<tpl
			if="parent.show['evalContentSections'+evaluation.version] && contentSections && contentSections.length &gt; 0">
		...
		<tpl
			if="parent.show['evalChecklistDetails'+evaluation.version] && checkListAll.responses && checkListAll.responses.length &gt; 0">
			<h3 class="quickView toggle-collapse">Evaluation Checklist Details <div data-qtip="Expand panel"
					style="float: right;" data-ref="toolEl"
					class=" x-tool-tool-el x-tool-img x-tool-expand-bottom eval-toggle-caret" role="presentation"></div>
			</h3>
			<table class="quickView-table" width="100%">
				...
				<tpl for="checkListAll.responses">
					<tr class="details-table">
						...
						<!-- Section -->
						<td class="details-table" style="text-align: center">{question.evaluationSectionDescription}
						</td>
