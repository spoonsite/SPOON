				...
				if (initialData) {
					...
					Ext.Array.each(data, function(item) {
						if (typeGroup[item.componentAttributePk.attributeType]) {
							...
							var preferredUnit = typeof item.preferredUnit === 'object' && item.preferredUnt !== null ? item.preferredUnit.unit : item.preferredUnit;
					...
					Ext.Object.each(typeGroup, function(key, value){
						Ext.Array.each(formPanel.items.items, function(field){
							if (field.attributeType === key) {
								...
								field.setValue(value * conversionFactor);
								field.setUnit(unit);
							}
						});
					});
