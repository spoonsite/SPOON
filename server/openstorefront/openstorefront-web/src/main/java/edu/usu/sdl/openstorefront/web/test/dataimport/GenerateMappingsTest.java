	...
	protected void runInternalTest()
	{
		...
		for (File originFile : dir.listFiles()) {
			if (originFile.isFile()) {
				...
				try (XMLMapReader mapReader = new XMLMapReader(new FileInputStream(originFile))) {
					...
					//remove some non-mapped paths
					unMappedFieldSet.remove("dataroot.noNamespaceSchemaLocation");
					unMappedFieldSet.remove("dataroot.generated");
					unMappedFieldSet.remove("dataroot");
					...
					for (String type : Arrays.asList("DATASENSITIVITY", "GEOMETRY", "PRODSTATUS")) {
						...
						if ("DATASENSITIVITY".equals(type)) {
							...
							fileAttributeCodeXrefMap.setExternalCode("In Development");
							fileAttributeTypeXrefMap.getAttributeCodeXrefMap().add(fileAttributeCodeXrefMap);

							fileAttributeCodeXrefMap = new FileAttributeCodeXrefMap();
							fileAttributeCodeXrefMap.setAttributeCode("INPRODUCTION");
							fileAttributeCodeXrefMap.setExternalCode("In Production");
							fileAttributeTypeXrefMap.getAttributeCodeXrefMap().add(fileAttributeCodeXrefMap);
					...
					//write out mapping			
					//String outFileName = "T:\\DI2E\\spoon\\mapping\\processed\\" + fileNameWithOutExt + ".json";
					//StringProcessor.defaultObjectMapper().writeValue(new File(outFileName), dataMapModel);
					
					//results.append("Wrote file: " + outFileName).append("<br>");										

					service.getImportService().saveFileDataMap(dataMapModel);
					
					results.append("Saved mapping: " + fileNameWithOutExt).append("<br>");										
				} catch (Exception ex) {
